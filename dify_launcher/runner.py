...
class ClaudeCodeRunner(Runner):
    ...
    def start(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        # Facts for this leg, written before the prompt is built and outside every state
        # mutation. Returns None on any write failure, which degrades to today's prompt.
        ctx = self._write_leg_context(job_id, start_label, pipeline)
    ...
    # Real director-skill filenames, each verified to exist on disk. NOT derivable from the
    # stage name: `scene_plan` -> `scene-plan-director.md` and `assets` -> `asset-director.md`,
    # so a templated f"{stage}-director.md" resolves to a file that is not there.
    _DIRECTOR_SKILLS = {
        "panda-video": {
            "script": "skills/pipelines/hybrid/script-director.md",
            "scene_plan": "skills/pipelines/panda-video/scene-plan-director.md",
            "assets": "skills/pipelines/panda-video/asset-director.md",
            "edit": "skills/pipelines/hybrid/edit-director.md",
            "compose": "skills/pipelines/panda-video/compose-director.md",
        },
        "panda-carousel": {
            "script": "skills/pipelines/panda-carousel/script-director.md",
            "scene_plan": "skills/pipelines/panda-carousel/scene-plan-director.md",
            "assets": "skills/pipelines/panda-carousel/asset-director.md",
        },
        "panda-image": {
            "scene_plan": "skills/pipelines/panda-image/scene-plan-director.md",
            "assets": "skills/pipelines/panda-image/asset-director.md",
        },
    }

    def _write_leg_context(self, job_id: str, stage: str, pipeline: str) -> Optional[Path]:
        """Write projects/{job}/CONTEXT.md — the facts this leg would otherwise rediscover.

        FACTS ONLY: absolute paths and where the job currently is, every value already held by
        the launcher. Deliberately NO gate numbers, NO partial_progress phases, NO pipeline
        shape — the director skills own those, and a prompt that restates them competes with
        the skill it is meant to point at. That is what made the earlier orientation attempt
        (dd33314) drive agents to do the wrong work, so this file states only what the launcher
        can derive and the agent cannot guess (lib/paths.py:19 makes the project dir unsafe to
        infer, and the resume prompt names no checkpoint path at all).

        Never raises, and returns None when it could not write: an orientation aid must not be
        able to fail a leg. Same contract as _record_timing above.
        """
        if not job_id:
            return None
        try:
            ...
            lines = [
                "# Leg context",
                "",
                "Generated by the launcher immediately before this leg. These are FACTS about",
                "where this job currently is — not instructions. Your stage's director skill",
                "remains the authority on what to do and when to stop.",
                "",
                f"- project dir (absolute): {proj}",
                f"- pipeline: {pipeline}",
                f"- stage this leg runs: {stage}",
            ]
            skill = (self._DIRECTOR_SKILLS.get(pipeline) or {}).get(stage)
            if skill:
                lines.append(f"- director skill for this stage: {skill}")
            cps = sorted(proj.glob("checkpoint_*.json"), key=lambda p: p.stat().st_mtime)
            if cps:
                lines.append(f"- newest checkpoint: {cps[-1]}")
            else:
                lines.append("- checkpoints: none yet — this is the job's first leg")
            out = proj / "CONTEXT.md"
            out.write_text("\n".join(lines) + "\n", encoding="utf-8")
            return out
        except OSError:
    ...
    def _start_prompt(self, job_id: str, brief: str, options: Optional[dict[str, Any]] = None,
                      pipeline: str = "panda-video",
                      context_path: Optional[Path] = None) -> str:
        options = options or {}
        ...
        # Points at the launcher-generated facts (paths / current position) so the leg does not
        # spend turns rediscovering them. Omitted entirely when the file could not be written —
        # the prompt must never name a path that is not there.
        ctx_line = (
            f"ORIENTATION — read {context_path} FIRST. The launcher generated it for this leg; "
            "it states the absolute project dir, the stage you are running, and that stage's "
            "director skill. It is fact, not instruction: the director skill it names remains "
            "the authority on what to do and when to stop.\n\n"
