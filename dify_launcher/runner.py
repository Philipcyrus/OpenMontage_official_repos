...
def _lip_sync_qa_line(options: Optional[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "Review every sampled mouth frame in order. Supply pre_speech_mouth_state plus one "
        "active_mouth_shapes label per active sample (closed|narrow|rounded|wide|teeth|unclear); "
        "the tool must reject continuous-open, two-shape, low-transition, and long static-mouth "
        "runs. Persist rubric_version='2.0', the ordered sequence, and derived metrics under "
        "asset_manifest.metadata.lip_sync_qa. "
...
class ClaudeCodeRunner(Runner):
    ...
    def _assets_approved_prompt(self, job_id: str, pipeline: Optional[str] = None) -> str:
        ...
        return (
            ...
            "final_review.checks.lip_sync_check; set final_review.status='warning' and top-level "
            "recommended_action='present_to_user' (never status='pass' with unresolved warnings); "
            "do not retry again or block. Checkpoint "
    ...
    def _edit_compose_continue_prompt(self, job_id: str,
        ...
        return (
            ...
            "status='warning' and recommended_action='present_to_user' (never status='pass'), "
            "and checkpoint compose status='awaiting_human'. "
