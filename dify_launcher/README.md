...
| GET  | `/health/canary` | last result of the daily Claude/Higgsfield cron canary (`deploy/panda_healthcheck.py`); read-only, never re-runs a check. **Dify should poll port 8502** (`deploy/panda_health_service.py`) instead — same shape, but it keeps answering when this launcher is down |
