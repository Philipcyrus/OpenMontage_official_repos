...
`approve_hero_still`, `approve_stills`, `approve_motion_sample`, and `approve_assets` are pauses of the **same** `assets`
stage (tell them apart by the `gate` field). `approve_hero_still` (one look-lock PNG) is **ON by default**;
pass `options.hero_still:false` to skip. `approve_motion_sample` (one hero clip, approve the
...
`start → approve_script → approve_scene_plan → approve_hero_still → approve_stills → approve_brand → done`. Optional
`options.gates: ["scene_plan", "stills"]` skips GATE 1. Opt out of look-lock with `hero_still:false`.
...
single hero PNG. At `approve_stills`, `artifacts.preview` is
