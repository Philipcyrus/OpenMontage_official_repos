...
# 2.5) approve scene_plan -> GATE 2.5 (approve_hero_still): ONE look-lock still ------------
...
hero_stills = b["artifacts"].get("stills")
assert hero_stills and len(hero_stills) == 1, f"hero gate must show ONE still, got {hero_stills}"
assert "storyboard" not in Path(hero_stills[0]).name.lower()
assert b["artifacts"].get("preview") == [f"/jobs/{job}/artifacts/{Path(hero_stills[0]).name}"], \
    f"hero preview must be the single PNG, got {b['artifacts'].get('preview')}"
assert "storyboard_html" not in b["artifacts"], "hero gate must not build the storyboard grid"
assert "clips" not in b["artifacts"]
assert not list(store.artifacts_dir(job).glob("*.mp4")), "hero gate must generate NO video"
print(f"   hero still gate: 1 still ({Path(hero_stills[0]).name}), look-lock before storyboard")

# revise hero still stays at hero gate (edit mode)
hero_d0 = _digest(job, hero_stills[0])
b = _step("respond revise hero mode=edit", c.post(f"/jobs/{job}/respond", json={
    "decision": "revise", "mode": "edit",
    "answer": "brighter panda, warmer lighting",
}), want_gate="approve_hero_still", want_status="awaiting_human")
hero_stills_e = b["artifacts"].get("stills")
assert hero_stills_e and len(hero_stills_e) == 1
assert _digest(job, hero_stills_e[0]) != hero_d0, "hero revise must rewrite the still"
print("   hero revise: stayed at approve_hero_still with one still")

# 3) approve hero -> GATE 3 (approve_stills): FULL STILLS, NO video yet -------------------
b = _step("respond approve (hero_still)", c.post(f"/jobs/{job}/respond", json={"decision": "approve"}),
...
c.post(f"/jobs/{job2}/respond", json={"decision": "approve"})            # -> hero_still
...
c.post(f"/jobs/{job3}/respond", json={"decision": "approve"})            # -> hero_still
...
c.post(f"/jobs/{jc}/respond", json={"decision": "approve"})              # -> hero_still
...
c.post(f"/jobs/{job4}/respond", json={"decision": "approve"})            # -> hero_still
b4 = _step("carousel hero still", c.get(f"/jobs/{job4}"),
           want_gate="approve_hero_still", want_status="awaiting_human")
assert b4["artifacts"].get("stills") and len(b4["artifacts"]["stills"]) == 1
assert "storyboard" not in Path(b4["artifacts"]["stills"][0]).name.lower()
assert b4["artifacts"].get("preview") == [
    f"/jobs/{job4}/artifacts/{Path(b4['artifacts']['stills'][0]).name}"]
...
c.post(f"/jobs/{b6['job_id']}/respond", json={"decision": "approve"})  # -> stills
...
# hero_still:false restores the old jump scene_plan → stills (video + carousel)
b_opt = _step("POST /jobs (hero_still=false)", c.post("/jobs", json={
    "brief": "skip look-lock",
    "options": {"hero_still": False, "motion_sample": False},
}), want_gate="approve_script", want_status="awaiting_human")
job_opt = b_opt["job_id"]
c.post(f"/jobs/{job_opt}/respond", json={"decision": "approve"})  # -> scene_plan
b_opt = _step("hero_still=false scene_plan -> stills",
              c.post(f"/jobs/{job_opt}/respond", json={"decision": "approve"}),
              want_gate="approve_stills", want_status="awaiting_human")
assert len(b_opt["artifacts"].get("stills") or []) == 3
assert b_opt["artifacts"].get("preview") == [f"/jobs/{job_opt}/artifacts/storyboard.png"]
print("   hero_still=false: scene_plan jumps straight to approve_stills")

b_copt = _step("POST /jobs (carousel hero_still=false)", c.post("/jobs", json={
    "brief": "carousel no look-lock",
    "pipeline": "panda-carousel",
    "options": {"hero_still": False, "gates": ["scene_plan", "stills"]},
}), want_gate="approve_scene_plan", want_status="awaiting_human")
b_copt = _step("carousel hero_still=false -> stills",
               c.post(f"/jobs/{b_copt['job_id']}/respond", json={"decision": "approve"}),
               want_gate="approve_stills", want_status="awaiting_human")
assert len(b_copt["artifacts"].get("stills") or []) == 3
print("   carousel hero_still=false: scene_plan jumps straight to approve_stills")
