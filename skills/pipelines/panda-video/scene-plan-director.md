...
1. **Subject** — who/what + key visual attributes; if multiple characters, how to disambiguate
   (panda vs customer). Name the reference **Element id** so the assets stage keeps them on-model.
...
5. **Camera** — capture in `shot_language`: `shot_size`, `camera_movement`, `lens_mm`,
   `lighting_key`, `depth_of_field`, `color_temperature`. Mark N/A explicitly for flat/text scenes.
...
- **`texture_keywords`** — texture descriptors where they help (`clean`, `soft`, `warm`, `glossy`).
...
the panda's on-model appearance, and the reference Element id from `config/panda-elements.json`
that the assets stage MUST use. Set the top-level `style_playbook` to the Panda style. Never render.
...
For each scene, list what the `assets` stage must produce, each as `{type, description, source}`
with `source` ∈ `generate | source | provided | record`. Descriptions must be actionable (not
"an image of the panda" but "Panda mascot waves at the airport gate, medium shot, warm key light,
on-model per panda Element"). Every `source: "generate"` asset must be feasible with the assets
